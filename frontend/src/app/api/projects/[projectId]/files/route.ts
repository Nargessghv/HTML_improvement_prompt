...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/projects/${projectId}/files${queryString ? '?' + queryString : ''}`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch project files from backend' },
    ...
    console.error('Error proxying project files request:', error)
    return NextResponse.json(
