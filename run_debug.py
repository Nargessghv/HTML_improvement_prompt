#!/usr/bin/env python
"""
Debug runner for slide generation workflow - Run from project root
...
class DebugRunner:
    ...
    def create_simple_outline(self, topic: str, description: str) -> dict:
    ...
    async def run_workflow(
        ...
        # Generate a project ID for tracking
        project_id = str(uuid.uuid4())
        print(f"\n📋 Project ID: {project_id}")
        
        # Create simple outline
        approved_outline = self.create_simple_outline(topic, description)
        print(f"📋 Created outline with {len(approved_outline['slides'])} slides")
...
async def main():
    ...
    try:
        # Set a breakpoint here to debug workflow initialization
